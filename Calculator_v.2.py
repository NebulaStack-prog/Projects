...
class Calculator:
    ...
    def create_left_panel(self, parent):
        ...
        categories = [
            ...
            ("#ff0000", "ФУНКЦИИ", [
                ("y = ax + b", self.line, 2),
                ("y = ax² + bx + c", self.parabola, 3),
                ("y = a / (x + b) + c", self.hiperbola, 3),
                ("y = a×cos(x) + b", self.cosinus, 2),
                ("y = a×sin(x) + b", self.sinus, 2),
                ("y = a×tg(x) + b", self.tang, 2)
            ]),

            ("#ff69b4", "ГРАФИКИ", [
                ("(y - a)² + (x - b)² = c²", self.circle, 3),
                ("y = a|x| + b", self.moduleX, 2),
                ("|y| = ax + b", self.moduleY, 2),
                ("|y| = a|x| + b", self.moduleXY, 2),
                ("y = a×ctg(x) + b", self.cotang, 2),
                ("Сердце", self.heart, 1)
            ])
        ]
    ...
    def line(self, a, b):
        ...
        plt.ahvline(x=0, color='black', linestyle='-', linewidth=2.5, label='Ось Y')
    ...
    def heart(self, a):
        ...
        plt.figure(figsize=(10, 10))
        plt.contour(X, Y, f, levels=[0], colors='#ff69b4', linewidth=10)
